...
export function placeOrder(restaurant, items) {
  ...
  return new Promise((resolve, reject) => {
    if (typeof restaurant !== "string" || restaurant.trim() === "" || !Array.isArray(items) || items.length === 0) {
...
export function confirmOrder(order) {
  // Your code here
  return new Promise((resolve, reject) => {
    if (!order || typeof order.orderId === "undefined" || order.status !== "placed") {
...
export function deliverOrder(order) {
  // Your code here
  return new Promise((resolve, reject) => {
    if (!order || typeof order.orderId === "undefined" || order.status !== "assigned" || !order.rider) {
...
export function processMultipleOrders(orderList) {
  // Your code here
  const orderPromises = orderList.map(order => processDelivery(order.restaurant, order.items));
